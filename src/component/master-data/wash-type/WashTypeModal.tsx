...
interface WashTypeModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (values: WashTypeItem) => void;
    defaultValues?: WashTypeItem;
    isSubmitting?: boolean;
}

export function WashTypeModal({ isOpen, onClose, onSubmit, defaultValues, isSubmitting = false }: WashTypeModalProps) {
    const form = useForm({
        defaultValues: {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Create or Update WashType" maxWidthClass="max-w-[440px]">
            <form
                onSubmit={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    <form.Subscribe selector={(state) => [state.canSubmit]}>
                        {([canSubmit]) => (
                            ...
                            >
                                ...
                                {isSubmitting ? 'Saving...' : 'Save WashType'}
                            </button>
                        )}
                    </form.Subscribe>
                </div>
